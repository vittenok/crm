...
export async function POST(request: Request) {
  try {
    ...
    const name = getValue(rawData, ["Name", "name", "Имя", "Ваше имя", "ФИО"]);
    const phone = getValue(rawData, ["Phone", "phone", "Телефон"]);
    const email = getValue(rawData, ["Email", "email", "Почта", "E-mail"]);
    ...
    const delivery = getValue(rawData, ["Radio", "Delivery", "Доставка"]);
